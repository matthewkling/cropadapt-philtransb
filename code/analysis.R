...
pxx <- td %>%
        ...
        geom_vline(data = tsd %>% mutate(var = toupper(var)), aes(xintercept = clim_delta), 
                   color = "black") +
        geom_hline(data = tsd %>% mutate(var = toupper(var)), aes(yintercept = crop_trend), 
                   color = "black") +
        ...
        labs(y = "trend in farm climate index", 
             x = "trend in climate variable (mm/yr)")
